...
### v0.8.0 (from v0.7.x)
- **All `remember_*` MCP tools removed.** The personal-memory layer (journals, collages, identities, whoami, user) moved to memberberry.ai. The 12 `briefing` / `remember_journal` / `remember_collage` / `remember_shared_collage` / `remember_user_journal` / `remember_whoami` / `user` / `remember_identity` / `directory` / `config` / `remember_audit` / `remember_search` tools no longer ship with the server.
...
- **Single `briefing` MCP tool replaces the 12 remember tools.** Same response shape as the old `briefing action=read`, no `action` arg.
